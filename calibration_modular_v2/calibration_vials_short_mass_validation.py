...
Usage:
  python calibration_modular_v2/calibration_vials_short_mass_validation.py          # Real hardware
  python calibration_modular_v2/calibration_vials_short_mass_validation.py --simulate   # Simulation mode
"""
...
def run_calibration_vials_short_mass_validation(simulate: bool = False) -> None:
    ...
    try:
        for idx, entry in enumerate(LIQUID_SERIES):
            label = entry["label"]
            liquid_name = entry["liquid_name"]
            vial_name = entry["vial_name"]
            
            # Show GUI only for first measurement for system verification
            show_gui_this_iteration = (idx == 0)
            ...
            # Show GUI for first measurement only
            cfg['experiment']['show_gui'] = show_gui_this_iteration
            if show_gui_this_iteration:
                logger.info("  >>> GUI enabled for first measurement (system verification)")
            
            # Initialize protocol for this liquid
            state = protocol.initialize(cfg)
            
            # Measure 3 replicates using protocol.measure()
            # Use standard defaults if not specified - protocol will use these with .get()
            params = {
                'parameters': {
                    'aspirate_speed': 10,
                    'dispense_speed': 10,
