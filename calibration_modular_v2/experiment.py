...
class CalibrationExperiment:
    ...
    def _generate_enhanced_outputs(self, results: ExperimentResults) -> None:
        ...
        try:
            ...
            generate_calibration_plots(trial_results, optimal_conditions, str(self.output_dir))
            ...
            if plots_dir.exists():
                ...
                logger.info(f"✅ Generated {plot_count} visualization plots in plots/")
            
            clean_csvs = [f for f in self.output_dir.glob("*_clean.csv")]
            if clean_csvs:
                logger.info(f"✅ Generated {len(clean_csvs)} clean CSV files")
            
            if insights:
                logger.info("✅ Generated analysis insights and recommendations")
