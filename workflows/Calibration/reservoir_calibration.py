...
# from tests.test_wp_stack import SIMULATE
...
lash_e = Lash_E("../utoronto_demo/status/sample_input_vials.csv", simulate=SIMULATE, initialize_biotek=False)

lash_e.nr_robot.check_input_file()

lash_e.nr_robot.prime_reservoir_line(1, overflow_vial=0, volume=1.0)
...
try:
    for volume in DISPENSE_VOLUMES: #loop through volumes to dispense
        ...
        if curr_volume + volume*REPLICATES > 8.0 and curr_vial_index < len(VIAL_INDICES) - 1: #move to next vial if not enough volume to add the next set of volumes *replicates
            lash_e.nr_robot.return_vial_home(curr_vial_index) #return current vial to home position
            curr_vial_index += 1 #moveto next vial
        ...
        for replicate in range(REPLICATES): #loop through number of replicates
            mass = lash_e.nr_robot.dispense_into_vial_from_reservoir(WATER_RESERVOIR_INDEX, curr_vial_index, volume, measure_weight=True, return_home=False)
    ...
    lash_e.nr_robot.return_vial_home(curr_vial_index)
