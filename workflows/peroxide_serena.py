...
def sample_workflow(input_vial_status_file,cytation_protocol_file_path):
    ...
    for _ in range (3):
        lash_e.nr_robot.aspirate_from_vial(1, 0.5)
        lash_e.nr_robot.dispense_into_vial(1, 0.5)
    ...
    for i in range (6, 12): 
        ...
        for _ in range (3):
            lash_e.nr_robot.aspirate_from_vial(i, 0.5)
            lash_e.nr_robot.dispense_into_vial(i, 0.5)
    ...
    for i in range (6, 12):
        ...
        if time.time() == next_addition_time:
    ...
    #Step 6: When the incubation is complete (18 min after each addition in step 5), add 200 µL "reaction mixture + diluted working reagent" (Vials 6-11) to the well plate & take absbance measurements.
            ...
            if current_time - incubation_tracker["start_time"] == next_plate_reading:
