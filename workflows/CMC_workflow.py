...
def create_wellplate_samples(lash_e, surfactant_mixture_index, surfactant_mixture_volume, DMSO_pyrene_index,DMSO_pyrene_volume,water_index,replicates,last_wp_index): #Add the DMSO_pyrene and surfactant mixture to well plates
    ...
    data = {'Volume A': [surfactant_mixture_volume]*replicates, 'Volume B': [DMSO_pyrene_volume]*replicates, 'Volume C': [1 - DMSO_pyrene_volume - surfactant_mixture_volume]*replicates}
...
def sample_workflow():
    ...
    surfactant_volumes = [0.6,0.9,1.0] #Note as written these can't exceed 1.0, but that's easy to change if you need larger volumes you can write a function that separates the volumes into <1 mL chunks
    DMSO_VOLUME = 0.10
    surfactant_mixture_volume = 0.2
    replicates=3

    mix_surfactants(lash_e, surfactant_index_list,surfactant_volumes,surfactant_mixture_index)
    create_wellplate_samples(lash_e, surfactant_mixture_index,surfactant_mixture_volume,pyrene_DMSO_index,DMSO_VOLUME,water_index,replicates=replicates,last_wp_index=LAST_WP_INDEX)
