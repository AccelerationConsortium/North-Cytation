...
def run_iterative_boundary_step(lash_e, adaptive_results, surfactant_a_name, surfactant_b_name, num_points=12):
    """
    Step 3: Iterative boundary finding - Add boundary points using existing stocks only.
    
    Args:
        lash_e: Lash_E coordinator instance
        adaptive_results: Results from adaptive step with existing data and stocks
        surfactant_a_name: First surfactant name  
        surfactant_b_name: Second surfactant name
        num_points: Number of new points to test (default: 12)
        
    Returns:
        dict: Updated adaptive_results with new boundary wells appended
    """
    lash_e.logger.info("=== STEP 3: ITERATIVE BOUNDARY FINDING ===")
    lash_e.logger.info(f"Adding {num_points} new points using existing stocks only")
    
    # Extract existing data and stock solutions
    existing_well_recipes_df = adaptive_results['well_recipes_df']
    existing_stock_solutions = adaptive_results['stock_solutions_needed']
    experiment_output_folder = adaptive_results.get('output_folder', 'output')
    
    lash_e.logger.info(f"Available data: {len(existing_well_recipes_df)} existing wells")
    lash_e.logger.info(f"Available substocks: {len(existing_stock_solutions)} stock solutions")
    
    # PHASE 1: Find new boundary points
    lash_e.logger.info("Phase 1: Finding boundary points using vector edge refinement...")
    ...
    try:
        ...
        from recommenders.generalized_vector_edge_recommender import (
            get_surfactant_boundary_recommendations
        )
        
        # Prepare existing data for analysis
        analysis_data = existing_well_recipes_df.copy()
        
        # Check if we have measurement columns
        if 'turbidity_600' not in analysis_data.columns or 'ratio' not in analysis_data.columns:
            lash_e.logger.error("Missing measurement data (turbidity_600, ratio) for boundary analysis")
            raise ValueError("Cannot perform boundary analysis without measurement results")
        
        analysis_data['well_type'] = 'experiment'
        temp_data_file = os.path.join(experiment_output_folder, 'existing_results_for_boundary_finding.csv')
        analysis_data.to_csv(temp_data_file, index=False)
        
        # Get boundary recommendations
        boundary_output_dir = os.path.join(experiment_output_folder, 'vector_edge_refinement')
        recommendations = get_surfactant_boundary_recommendations(
            temp_data_file, 
            n_points=num_points,
        ...
        lash_e.logger.info(f"Generated {len(recommendations)} boundary recommendations")
        
        # Check if boundary points can be made from existing stocks using real stock selection
        lash_e.logger.info("Checking if new boundary points can be made from existing stocks...")
        
        # Create substock tracker and load existing stocks
        tracker = SurfactantSubstockTracker()
        for stock in existing_stock_solutions:
            if stock['source_vial'] != 'Unknown':  # This is a substock, not original stock
                tracker.substocks[stock['vial_name']] = {
                    'surfactant_name': stock['surfactant'],
                    'concentration_mm': stock['target_concentration_mm'],
                    'volume_ml': 10.0  # Assume plenty available
                }
        
        # Try to find stock solutions for each boundary recommendation
        valid_well_recipes = []
        for idx, row in recommendations.iterrows():
            surf_a_conc = row['surf_A_conc_mm']
            surf_b_conc = row['surf_B_conc_mm']
            
            # Use real stock selection method
            solution_a = tracker.find_best_solution_for_concentration(surfactant_a_name, surf_a_conc)
            solution_b = tracker.find_best_solution_for_concentration(surfactant_b_name, surf_b_conc)
            
            if solution_a and solution_b:
                # Calculate water volume
                total_surfactant_volume = solution_a['volume_needed_ul'] + solution_b['volume_needed_ul']
                water_volume = WELL_VOLUME_UL - total_surfactant_volume - (BUFFER_VOLUME_UL if ADD_BUFFER else 0)
                
                if water_volume >= 0:
                    recipe = {
                        'wellplate_index': len(valid_well_recipes),
                        'well_type': 'experiment',
                        'control_type': 'experiment',
                        'surf_A': surfactant_a_name,
                        'surf_B': surfactant_b_name,
                        ...
                        'substock_B_conc_mm': solution_b['concentration_mm'],
                        'surf_B_volume_ul': solution_b['volume_needed_ul'],
                        'water_volume_ul': max(0, water_volume),
                        'buffer_volume_ul': BUFFER_VOLUME_UL if ADD_BUFFER else 0.0,
                        'buffer_used': SELECTED_BUFFER if ADD_BUFFER else None,
                    ...
                    valid_well_recipes.append(recipe)
                    lash_e.logger.info(f"  ✓ surf_A={surf_a_conc:.4f}mM ({solution_a['vial_name']}, {solution_a['volume_needed_ul']:.1f}uL), surf_B={surf_b_conc:.4f}mM ({solution_b['vial_name']}, {solution_b['volume_needed_ul']:.1f}uL)")
                else:
                    lash_e.logger.warning(f"  ✗ surf_A={surf_a_conc:.4f}mM, surf_B={surf_b_conc:.4f}mM - negative water volume")
            else:
                missing_a = "no stock" if not solution_a else "OK"
                missing_b = "no stock" if not solution_b else "OK"
                lash_e.logger.warning(f"  ✗ surf_A={surf_a_conc:.4f}mM ({missing_a}), surf_B={surf_b_conc:.4f}mM ({missing_b})")
        
        if len(valid_well_recipes) == 0:
            raise ValueError("Cannot create boundary points - all required concentrations need new stocks that don't exist")
        
        # Convert to DataFrame
        new_well_recipes_df = pd.DataFrame(valid_well_recipes)
        
        lash_e.logger.info(f"Created {len(new_well_recipes_df)} new experimental conditions from existing stocks")
        ...
        lash_e.logger.error(f"Error in boundary point generation: {str(e)}")
        raise ValueError(f"Cannot generate boundary points from existing stocks: {str(e)}")
        
    if len(new_well_recipes_df) == 0:
        raise ValueError("Cannot proceed - no boundary points can be made from available stocks")
    
    # PHASE 2: Execute new points
    lash_e.logger.info(f"Phase 2: Executing {len(new_well_recipes_df)} new boundary points...")
    
    # Add necessary columns
    new_well_recipes_df['surf_A_name'] = surfactant_a_name
    new_well_recipes_df['surf_B_name'] = surfactant_b_name
    new_well_recipes_df['step'] = 'iterative_boundary'
    
    # Execute dispensing and measurements
    wellplate_data = {
        'current_plate': 1, 
        'wells_used': len(existing_well_recipes_df),
        'last_measured_well': -1,
        'measurements': []
    }
    
    measured_well_recipes_df = execute_wellplate_dispensing_and_measurements(
        lash_e, new_well_recipes_df, wellplate_data
    )
    
    # Add iteration tracking to new wells
    measured_well_recipes_df['iteration'] = 'boundary_iteration_1'
    measured_well_recipes_df['step'] = 'iterative_boundary'
    
    # Add iteration column to existing wells if not present
    if 'iteration' not in existing_well_recipes_df.columns:
        existing_well_recipes_df['iteration'] = 'adaptive_initial'
    
    # Combine DataFrames
    combined_well_recipes_df = pd.concat([existing_well_recipes_df, measured_well_recipes_df], ignore_index=True)
    
    lash_e.logger.info(f"Appending {len(measured_well_recipes_df)} boundary wells to existing {len(existing_well_recipes_df)} wells")
    
    # Save simple CSV backup (no heavy visualization)
    boundary_csv_path = os.path.join(experiment_output_folder, 'boundary_iteration_results.csv')
    measured_well_recipes_df.to_csv(boundary_csv_path, index=False)
    lash_e.logger.info(f"Saved boundary iteration results: {boundary_csv_path}")
    
    # Update adaptive_results with combined data
    adaptive_results['well_recipes_df'] = combined_well_recipes_df
    adaptive_results['total_wells'] = len(combined_well_recipes_df)
    adaptive_results['boundary_wells_added'] = len(measured_well_recipes_df)
    adaptive_results['last_iteration'] = 'boundary_iteration_1'
    adaptive_results['iterations_completed'] = 1
    
    lash_e.logger.info(f"Updated experiment data: {len(combined_well_recipes_df)} total wells ({len(existing_well_recipes_df)} adaptive + {len(measured_well_recipes_df)} boundary)")
...
if __name__ == "__main__":
    ...
    # if not SIMULATE:
    #     slack_agent.send_slack_message("Completed adaptive surfactant grid screening workflow...")rrecommecommender. s