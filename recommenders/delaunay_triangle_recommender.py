...
class DelaunayTriangleRecommender:
    ...
    def _normalize_outputs(self, experiment_data):
        ...
        for j, col in enumerate(self.output_columns):
            ...
            if self.normalization_method == 'log_zscore':
                ...
                print(f"  {col}: {values.min():.4f}-{values.max():.4f} ΓåÆ log ΓåÆ z-score: {normalized.min():.3f}-{normalized.max():.3f}")
                ...
                print(f"  {col}: {values.min():.4f}-{values.max():.4f} ΓåÆ z-score: {normalized.min():.3f}-{normalized.max():.3f}")
                ...
                print(f"  {col}: {values.min():.4f}-{values.max():.4f} ΓåÆ minmax: {normalized.min():.3f}-{normalized.max():.3f}")
    ...
    def _build_triangulation(self, U):
        ...
        try:
            ...
            print(f"  Built triangulation: {len(U)} points ΓåÆ {len(triangles)} triangles")
        ...
        try:
            ...
            for output_var in self.output_columns:
                ...
                legend_elements = [
                    ...
                    Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=10, label='Score Γëñ 0.1')
                ]
