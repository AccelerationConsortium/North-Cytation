...
class VialEditDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # Button box with Remove and Empty options
        ...
        self.empty_button = QPushButton("Empty Vial")
        self.empty_button.setStyleSheet("QPushButton { background-color: #fff3cd; color: #856404; }")
        self.empty_button.setToolTip("Quickly set volume to 0 and close dialog")
        ...
        self.empty_button.clicked.connect(self.empty_vial)
    ...
    def empty_vial(self):
        """Quickly set volume to 0 and close dialog."""
        # Set volume to 0
        self.volume_spin.setValue(0.0)
        # Immediately accept the dialog to save and close
        self.accept()
...
class VialManagerMainWindow(QMainWindow):
    ...
    def _return_all_home(self):
        ...
        if not hasattr(self, 'original_vials_data') or not self.original_vials_data:
            ...
            QMessageBox.warning(self, "No Data", "No vials loaded. Please load a status file first.")
            return
        ...
        print(f"[DEBUG] _return_all_home: Starting with {total_vials} vials")
        ...
        try:
            ...
            for i, vial_data in enumerate(self.original_vials_data):
                ...
                current_index = vial_data.get('location_index', 0)
                home_index = vial_data.get('home_location_index', 0)
                
                debug_info.append(f"{vial_name}: {current_location}[{current_index}] -> {home_location}[{home_index}]")
                
                # If vial is not at home location, move it digitally
                location_changed = current_location and home_location and current_location != home_location
                index_changed = current_index != home_index
                
                if location_changed or index_changed:
                    # Update the master data structure directly
                    self.original_vials_data[i]['location'] = home_location or current_location
                    ...
                    changes_made += 1
                    debug_info.append(f"  -> MOVED: {vial_name} from {current_location}[{current_index}] to {home_location}[{home_index}]")
            ...
            if changes_made > 0:
                ...
                print(f"[DEBUG] Calling _reload_all_widgets to refresh display...")
    ...
    def _reload_all_widgets(self):
        ...
        try:
            print(f"[DEBUG] _reload_all_widgets: Reloading {len(self.rack_widgets)} rack widgets...")
            
            # Clear and reload each rack widget type
            for rack_name, rack_widget in self.rack_widgets.items():
                print(f"[DEBUG] Reloading rack: {rack_name}, type: {type(rack_widget).__name__}")
                
                if hasattr(rack_widget, 'add_vials'):
                    print(f"[DEBUG] Calling add_vials for {rack_name} with {len(self.original_vials_data)} vials")
                    # For VialRackWidget - clear existing widgets first
                    if hasattr(rack_widget, 'vials'):
                        # Clear existing vial widgets
                        for location_index, vial_widget in rack_widget.vials.items():
                            vial_widget.deleteLater()
                    ...
                    # For CombinedRackWidget (auxiliary widgets)
                    print(f"[DEBUG] Updating vial_data_list for {rack_name}")
                    location_vials = [v for v in self.original_vials_data 
                                    if v.get('location') in ['large_vial_rack', 'photoreactor_array', 'clamp']]
                    print(f"[DEBUG] Found {len(location_vials)} auxiliary vials")
                    
                    # Clear existing widgets
                    if hasattr(rack_widget, 'vials'):
                        for key, vial_widget in rack_widget.vials.items():
                            vial_widget.deleteLater()
                        rack_widget.vials.clear()
                        rack_widget.vial_data_list.clear()
                    ...
                    print(f"[DEBUG] Unknown widget type for {rack_name}, skipping")
            
            print(f"[DEBUG] _reload_all_widgets: Completed successfully")
                        
        except Exception as e:
            print(f"[ERROR] Failed to reload some widgets: {e}")
            import traceback
            traceback.print_exc()
