...
# If buffer is added, reduce surfactant volumes proportionally to maintain concentrations
if ADD_BUFFER:
...
def get_achievable_concentrations(surfactant_name, target_concentrations):
    ...
    for target in target_concentrations:
        ...
        if dilution_factor >= 1.0 and dilution_factor <= MAX_DILUTION_FACTOR:
...
def pipette_grid_to_shared_wellplate(lash_e, concs_a, concs_b, dilution_vials_a, dilution_vials_b, surfactant_a_name, surfactant_b_name, shared_wellplate_state):
    ...
    for batch_start in range(0, len(all_well_requirements), MEASUREMENT_INTERVAL):
        ...
        for vial_name, well_list in sorted_vial_groups_b:
            print(f"    Using vial {vial_name} for {len(well_list)} wells")
            for batch_idx, actual_well, req in well_list:
                volume_each = EFFECTIVE_SURFACTANT_VOLUME / 1000  # Convert uL to mL
        ...
        # PHASE 2.5: Add buffer (optional)
        if ADD_BUFFER:
            print(f"  Phase 2.5: Adding {SELECTED_BUFFER} buffer ({BUFFER_VOLUME_UL} uL) to wells {wells_in_batch[0]}-{wells_in_batch[-1]}")
            ...
            try:
                ...
                logger.warning(f"Buffer vial '{SELECTED_BUFFER}' not found in vial status - skipping buffer addition")
                print(f"Warning: Buffer vial '{SELECTED_BUFFER}' not found - continuing without buffer")
            else:
                # Add buffer to all wells in batch using single tip
                total_buffer_ml = buffer_volume_ml * len(wells_in_batch)
                lash_e.nr_robot.aspirate_from_vial(SELECTED_BUFFER, total_buffer_ml, liquid='water', use_safe_location=True)
...
def save_results(results_df, well_map, wellplate_data, all_measurements, concentrations, dilution_vials_a, dilution_vials_b, dilution_steps_a, dilution_steps_b, surfactant_a_name, surfactant_b_name, simulate=True, output_folder=None, logger=None):
    ...
    with open(dilution_report_file, 'w') as f:
        ...
        f.write(f"Each well contains {EFFECTIVE_SURFACTANT_VOLUME:.1f} uL of surfactant A dilution\n")
        f.write(f"                 + {EFFECTIVE_SURFACTANT_VOLUME:.1f} uL of surfactant B dilution\n")
        if ADD_BUFFER:
            f.write(f"                 + {BUFFER_VOLUME_UL} uL of {SELECTED_BUFFER} buffer\n")
        f.write(f"                 = {EFFECTIVE_SURFACTANT_VOLUME * 2 + (BUFFER_VOLUME_UL if ADD_BUFFER else 0):.1f} uL total per well\n")
    ...
    dilution_info = {
        ...
        'well_mixing': {
            'volume_each_ul': EFFECTIVE_SURFACTANT_VOLUME,
            'total_surfactant_volume_ul': EFFECTIVE_SURFACTANT_VOLUME * 2,
            'buffer_added': ADD_BUFFER,
            'buffer_type': SELECTED_BUFFER if ADD_BUFFER else None,
            'buffer_volume_ul': BUFFER_VOLUME_UL if ADD_BUFFER else 0
        }
    }
    ...
    metadata = {
        ...
        'effective_surfactant_volume_ul': EFFECTIVE_SURFACTANT_VOLUME,
        'buffer_added': ADD_BUFFER,
        'buffer_type': SELECTED_BUFFER if ADD_BUFFER else None,
        'buffer_volume_ul': BUFFER_VOLUME_UL if ADD_BUFFER else 0,
