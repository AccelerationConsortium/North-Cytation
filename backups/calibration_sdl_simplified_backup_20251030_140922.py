...
# DEFAULT_REPLICATES = 1  # DEPRECATED: Replaced by PRECISION_MEASUREMENTS system
...
def optimize_subsequent_volume_budget_aware(volume, lash_e, state, autosave_raw_path, raw_measurements, 
    ...
    if deviation <= deviation_threshold and current_measurements + PRECISION_MEASUREMENTS - 1 <= measurements_budget:
        ...
        # Check if inherited parameters meet tolerance
        if avg_deviation <= tolerances['tolerance_percent'] and variability <= tolerances['variation_ul']:
    ...
    if volume_results:
        ...
        tolerance_met = (best_result['deviation'] <= tolerances['tolerance_percent'] and 
                        best_result['variability'] <= tolerances['variation_ul'])
...
def run_budget_constrained_optimization(volume, lash_e, state, autosave_raw_path, raw_measurements,
    ...
    """
    Run optimization within measurement budget using existing adaptive measurement logic.
    ...
    # Check if single-objective optimizer is available for deviation-focused optimization
    if not OPTIMIZER_SINGLE_AVAILABLE:
        print("   ⚠️  Single-objective optimizer not available - using inherited parameters")
        return []
    
    # Create single-objective optimizer focusing on deviation
    tip_volume = get_tip_volume_for_volume(lash_e, volume)
    max_overaspirate_ul = get_max_overaspirate_ul(volume)
    ...
    while get_volume_measurement_count(optimization_start_count) < budget:
        try:
            ...
            # Early termination if we found good enough result
            if (adaptive_result['deviation'] <= tolerances['tolerance_percent'] and 
                adaptive_result['variability'] <= tolerances['variation_ul']):
...
if __name__ == "__main__":
    ...
    optimal_conditions, save_dir = run_simplified_calibration_workflow(
        ...
        sim_dev_multiplier=1.5,  # Moderate challenge
        sim_var_multiplier=1.5   # Moderate challenge
    )
