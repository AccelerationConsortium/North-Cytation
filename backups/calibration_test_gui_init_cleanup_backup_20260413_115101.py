...
class ParameterWidget(QFrame):
    ...
    def setup_ui(self):
        ...
        # Add speed inversion reminder for speed parameters
...
class PlotWidget(QFrame):
    ...
    def plot_individual_measurements(self, results: Dict[str, Any]):
        ...
        for i, (vol, y_pos) in enumerate(zip(volumes_ul, y_jitter)):
            ax.annotate(f'R{i+1}\\n{vol:.1f}', (vol, y_pos), 
...
class CalibrationTestMainWindow(QMainWindow):
    ...
    def create_input_column(self) -> QWidget:
        ...
        self.cleanup_btn = QPushButton("CLEANUP & HOME")
    ...
    def cleanup_protocol(self):
        ...
        try:
            ...
            if hasattr(self.protocol, 'cleanup') and self.protocol_state:
                self.protocol.cleanup(self.protocol_state)
    ...
    def plot_individual_measurements_progressive(self):
        ...
        # Add accuracy info in corner
        if n_measurements > 0:
            deviation_pct = abs(np.mean(volumes_ul) - target_ul) / target_ul * 100
            cv_pct = (np.std(volumes_ul) / np.mean(volumes_ul) * 100) if n_measurements > 1 else 0
            info_text = f'Accuracy: {deviation_pct:.1f}% dev\\nPrecision: {cv_pct:.1f}% CV'
