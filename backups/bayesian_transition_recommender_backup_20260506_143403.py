...
class BayesianTransitionRecommender(TransitionRecommenderBase):
    ...
    Inherits data plumbing (input/output processing, metrics, plotting,
    `get_recommendations` orchestration) from TransitionRecommenderBase.
    Implements the local-contrast acquisition function as `_propose_batch`.
    """
    ...
    def __init__(self, input_columns: List[str], output_columns: List[str],
        ...
        lam : float
            Exploration bonus weight (lambda in acquisition function).
        min_distance : float
        ...
        print(f"Initialized BayesianTransitionRecommender (local-contrast acquisition):")
    ...
    def _compute_adaptive_min_distance(self, n_existing: int, n_batch: int) -> float:
        ...
        print(f"    Adaptive spacing: n_total={n_total}, d={self.n_inputs}D -> "
              f"uniform={uniform_spacing:.4f}, alpha={ALPHA_SPACING} -> min_dist={final_min_dist:.4f}")
    ...
    def _propose_batch(self, models: ModelListGP, X_existing: torch.Tensor,
        ...
        Note: ``self.min_distance`` is consulted but ``_compute_adaptive_min_distance``
        overrides it when called.
        """
        # min_distance kept for parity with prior call signature; no-op pass-through
        requested_min_distance = self.min_distance
...
def main():
    ...
    for boundary_name, boundary_func, description in boundary_types[:2]:  # Test first 2 types
        ...
        print(f"    Local contrast: delta={DELTA:.3f}, K={K}")
        print(f"    Exploration: lambda={LAM:.2f}")
