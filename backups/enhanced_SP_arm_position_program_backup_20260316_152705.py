...
class EnhancedRobotArmController:
    ...
    def _create_mock_robot(self):
        ...
        class MockRobot:
            ...
            def get_robot_positions(self):
                return [
                    int(self.gripper_angle_rad * 1000),
    ...
    def create_gui(self):
        ...
        save_temp_button = ttk.Button(modifications_frame, text="Save Current as Temp", 
                                     command=self.save_temporary_position)
        save_temp_button.grid(row=1, column=0, padx=(0, 5), pady=2)
        
        reset_button = ttk.Button(modifications_frame, text="Reset to Original", 
                                 command=self.reset_to_original_position)
        reset_button.grid(row=1, column=1, padx=(0, 5), pady=2)
        
        clear_temp_button = ttk.Button(modifications_frame, text="Clear All Temp", 
                                      command=self.clear_temporary_positions)
        clear_temp_button.grid(row=1, column=2, pady=2)
        ...
        ttk.Button(export_frame, text="Export Temp Positions (CSV)", 
                  command=self.export_temporary_positions_csv).grid(row=0, column=0, padx=(0, 5))
        ttk.Button(export_frame, text="Export Session History (TXT)", 
                  command=self.export_session_history).grid(row=0, column=1, padx=(0, 5))
        ttk.Button(export_frame, text="Export All Positions (JSON)", 
                  command=self.export_all_positions_json).grid(row=0, column=2)
