...
class EnhancedRobotArmController:
    ...
    def move_x_left(self):
        """Move X-axis left (negative X direction)."""
    ...
    def _safe_move_xy(self, target_x, target_y):
        """Safely move to X-Y coordinates with workspace bounds checking."""
        if not self.is_connected:
            return
            
        # Basic workspace bounds checking (robot can reach roughly -340 to +340 mm)
        WORKSPACE_X_MIN, WORKSPACE_X_MAX = -300, 300  # Conservative bounds
        WORKSPACE_Y_MIN, WORKSPACE_Y_MAX = -300, 300
        
        target_x = max(WORKSPACE_X_MIN, min(WORKSPACE_X_MAX, target_x))
        target_y = max(WORKSPACE_Y_MIN, min(WORKSPACE_Y_MAX, target_y))
        
        try:
            logger.info(f"Moving to X={target_x:.1f}mm, Y={target_y:.1f}mm")
            self.robot.move_xy(target_x, target_y, wait=True)
            self.current_x_position = target_x
            self.current_y_position = target_y
            self.update_display()
        except Exception as e:
            logger.error(f"Error moving to X-Y position: {str(e)}")
            # Try to update current position from robot
            try:
                positions = self.robot.get_robot_positions()
                cartesian_pos = self.robot.n9_fk(positions[0], positions[2], positions[1])
                self.current_x_position = cartesian_pos[0]
                self.current_y_position = cartesian_pos[1]
                self.update_display()
            except Exception as update_error:
                logger.warning(f"Could not update position after failed move: {update_error}")
