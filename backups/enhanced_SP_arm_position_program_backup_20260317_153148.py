...
class EnhancedRobotArmController:
    ...
    def create_gui(self):
        ...
        ttk.Button(control_frame, text="← SHOULDER -", command=self.move_shoulder_negative).grid(row=2, column=1, padx=2)
        ttk.Button(control_frame, text="→ SHOULDER +", command=self.move_shoulder_positive).grid(row=2, column=2, padx=2)
        
        ttk.Label(control_frame, text="Elbow:").grid(row=3, column=0, sticky=tk.W)
        ttk.Button(control_frame, text="↓ ELBOW -", command=self.move_elbow_negative).grid(row=3, column=1, padx=2)
        ttk.Button(control_frame, text="↑ ELBOW +", command=self.move_elbow_positive).grid(row=3, column=2, padx=2)
    ...
    def move_shoulder_negative(self):
        """Move shoulder in negative direction."""
        target_angle = self.current_shoulder_angle - self.move_increment_rad
        self._safe_move_shoulder(target_angle)
        
    def move_shoulder_positive(self):
        """Move shoulder in positive direction."""
        target_angle = self.current_shoulder_angle + self.move_increment_rad
        self._safe_move_shoulder(target_angle)
        
    def move_elbow_negative(self):
        """Move elbow in negative direction."""
        target_angle = self.current_elbow_angle - self.move_increment_rad
        self._safe_move_elbow(target_angle)
        
    def move_elbow_positive(self):
        """Move elbow in positive direction."""
        target_angle = self.current_elbow_angle + self.move_increment_rad
        self._safe_move_elbow(target_angle)
