...
class ConstraintCalibrator:
    ...
    def calculate_two_point_bounds(
        ...
        if best_trial_variability_ml is not None and best_trial_variability_ml > 0:
            # Use variability from best trial directly as requested
            min_overaspirate_ml -= best_trial_variability_ml
            max_overaspirate_ml += best_trial_variability_ml
            logger.info(f"Applied simple variability adjustment: +/-{best_trial_variability_ml*1000:.1f}uL from best trial")
        else:
