...
# Constants
FINAL_SUBSTOCK_VOLUME_ML = 6.0  # mL final volume for each substock dilution (reduced from 7.5)
...
def execute_kinetics_workflow(surfactant_a_name="SDS", surfactant_b_name="DTAB",
    ...
    lash_e.logger.info(f"Dispensing complete: {len(well_recipes_df)} wells prepared")
        ...
        # PHASE 2: Execute measurement sequence using existing function (eliminates hundreds of lines of duplication)
    ...
    return {
        'surfactant_a': surfactant_a_name,
        'surfactant_b': surfactant_b_name,
        'well_recipes_df': sequence_results['well_recipes_df'],
        'dispensing_results': dispensing_results,
        'output_folder': sequence_results['output_folder'],
...
def execute_single_kinetics_sequence(sequence, dispensing_results, surfactant_a_name, surfactant_b_name,
    ...
    return {
        ...
        'results_path': experiment_output_folder,
...
def get_suggested_concentrations(experiment_data_df, surfactant_a_name, surfactant_b_name, n_suggestions=GRADIENT_SUGGESTIONS_PER_ITERATION, output_dir=None, iteration_number=None):
    ...
    # CRITICAL FIX: Create mask from experimental data only to match triangle recommender indexing
    output_reliability = None
    if FILTER_UNRELIABLE_RATIOS and OPTIMIZE_METRIC in ['ratio', 'both']:
        # Get experimental points only (same filtering as triangle recommender does internally)
        if 'well_type' in data_for_recommender.columns:
            experimental_only = data_for_recommender[data_for_recommender['well_type'] == 'experiment'].copy()
        else:
            experimental_only = data_for_recommender.copy()
            
        n_points = len(experimental_only)
        ...
        for i, output_col in enumerate(output_columns):
            if output_col == 'ratio':
                ...
                unreliable_mask = (
                    (experimental_only.get('turbidity_600', 0) > TURBIDITY_FILTER_THRESHOLD) |
                    (experimental_only[output_col] > HIGH_RATIO_THRESHOLD)
                )
...
if __name__ == "__main__":
    ...
    # Create experiment folder - different logic for double vs single workflows
    if WORKFLOW_TYPE == 'double_iterative':
