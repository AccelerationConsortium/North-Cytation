...
def check_stopping_criteria(all_results, volume_ml, tolerances):
    ...
    1. Reached MAX_TRIALS total individual measurements, OR
    2. Have MIN_GOOD_TRIALS "good" individual measurements
    
    Note: Each individual measurement counts as a trial, so replicates count separately.
    ...
    # Count total individual measurements (trials) - each replicate counts separately
    total_trials = 0
    good_trials = 0
    
    for result in first_volume_results:
        # Each result represents a parameter set that may have multiple replicates
        replicate_count = result.get('replicate_count', 1)  # Default to 1 if not specified
        total_trials += replicate_count
        
        # Check if this parameter set produced good results
        quality = evaluate_trial_quality(result, volume_ml, tolerances)
        if quality['is_good']:
            # If the parameter set is good, all its replicates count as good trials
            good_trials += replicate_count
    
    # Check stopping criteria
    if total_trials >= MAX_TRIALS:
        return {
            'should_stop': True,
            'reason': f'Reached maximum individual measurements ({MAX_TRIALS})',
        ...
        return {
            'should_stop': True,
            'reason': f'Reached minimum good measurements ({MIN_GOOD_TRIALS})',
            'good_trials': good_trials,
            'total_trials': total_trials
        }
    else:
        return {
            'should_stop': False,
            'reason': f'Need {MIN_GOOD_TRIALS - good_trials} more good measurements or {MAX_TRIALS - total_trials} more total measurements',
            'good_trials': good_trials,
            'total_trials': total_trials
        }
...
def optimize_subsequent_volume(volume, lash_e, state, autosave_raw_path, raw_measurements, 
    ...
    # Check if v2 optimizer is available
    if not RECOMMENDER_V2_AVAILABLE:
        print("❌ v2 optimizer not available - cannot proceed with subsequent volume optimization")
        return False, None
    
    # Create 2-objective optimizer (deviation + time) - v2 doesn't support selective optimization
    # We'll handle parameter fixing manually in the suggestions
    ...
    # Store fixed parameters for manual combination later
    ...
    while True:
        ...
        # Get next suggestion - v2 only optimizes volume-dependent params, we combine with fixed
        optimized_params, trial_index = ax_client.get_next_trial()
        
        # Combine optimized parameters with fixed parameters
        params = dict(fixed_params)  # Start with fixed parameters
        params.update(optimized_params)  # Add optimized parameters
        
        optimization_trial_count += 1
        
        print(f"   Optimization trial {optimization_trial_count}...")
        
        # Run single trial (no adaptive measurement for subsequent volumes)
        check_if_measurement_vial_full(lash_e, state)
        liquid_source = get_liquid_source_with_vial_management(lash_e, state)
        ...
        # Add result to model (2-objective: deviation + time only)
        model_result = {
            "deviation": result.get('deviation', 0),
            "time": result.get('time', 0)
        }
