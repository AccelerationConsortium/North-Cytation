...
class ConstraintCalibrator:
    ...
    def calculate_two_point_bounds(
        ...
        # Order points by overaspirate volume (not measured volume)
        # We want to understand: overaspirate_low -> overaspirate_high effect on volume
        if point_1.overaspirate_vol_ml > point_2.overaspirate_vol_ml:
            # Swap if Point 1 has higher overaspirate
            high_point = point_1  # Higher overaspirate
            low_point = point_2   # Lower overaspirate
        else:
            # Keep as is if Point 2 has higher overaspirate
            high_point = point_2  # Higher overaspirate  
            low_point = point_1   # Lower overaspirate
